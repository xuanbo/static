...
# 编译成可执行程序
RUN go build -mod=vendor -o bin/app cmd/static/main.go
...
# 复制可执行文件、配置文件
COPY --from=builder /apps/static/bin/app bin/
COPY --from=builder /apps/static/static static/

# 暴露端口
EXPOSE 12345

# 挂载
VOLUME ["/apps/static/project"]
